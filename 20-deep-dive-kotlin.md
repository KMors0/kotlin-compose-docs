...
>
> **`split(" ")`** — вызов метода `split` на `this` (строке). Без явного `this.split(" ")` компилятор понимает это автоматически.
>
> **`joinToString(" ") { ... }`** — функция, объединяющая элементы коллекции в строку с разделителем `" "`. Принимает лямбду, которая преобразует каждый элемент.
>
> **`it`** в лямбде `{ it.replaceFirstChar { ... } }` — каждый элемент коллекции (в данном случае слово из исходной строки). `it` — автоматическое имя единственного параметра лямбды.
>
> **Вложенная лямбда** `{ c -> c.uppercase() }` — здесь параметр назван явно (`c`), а не `it`. Вложенные `it` конфликтовали бы, поэтому для внутренней лямбды даём своё имя.
>
> **`replaceFirstChar { ... }`** — функция, заменяющая первый символ строки на результат лямбды.
