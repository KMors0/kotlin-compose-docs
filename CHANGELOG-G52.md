...
# Часть 6: концептуальное погружение в программирование (30 июня 2026)

Пользователь запросил создать **полноценное погружение в программирование** — не справочник «вот так делают класс, вот так функцию», а концептуальное объяснение: что такое функция (это исполняемый кусок кода, который может...), что такое класс (это каркас, на основе которого мы можем создать экземпляр...). Подход «от понимания к синтаксису», а не «от синтаксиса к пониманию».

## Что создано

### Новая глава: `00-programming-foundations.md` (827 строк)

Глава идёт **самой первой** в обучении — перед словарём программиста и введением. Целевая аудитория: те, кто никогда не программировал и хочет понять концепции, а не просто выучить синтаксис.

### Структура главы (14 разделов)

1. **Что такое программа?** — аналогия с рецептом блюда, объяснение зачем нужны языки программирования и компиляторы.
2. **Переменная — «коробка с именем»** — что такое переменная, зачем имена, `val` vs `var`, правило «предпочитай val».
3. **Тип данных — «контракт»** — почему типы важны, основные типы Kotlin (Int, String, Boolean, Double и т.д.), статическая типизация.
4. **Функция — «именованный исполняемый кусок кода»** — полная анатомия: `fun`, имя, параметры, возвращаемое значение, тело. Аналогия с кнопкой на пульте. Подробное объяснение как работает вызов функции (пошагово). Функции высшего порядка и лямбды.
5. **Класс — «чертёж» для создания объектов** — аналогия с архитектурным планом, поля и методы (состояние и поведение), конструктор, `BankAccount` как пример с реальной логикой.
6. **Объект — «экземпляр класса»** — что происходит при `Dog("Rex", 3)` (выделение памяти, конструктор, ссылка). `==` vs `===`, `data class`.
7. **Условия — «выбор пути»** — `if/else`, `when` как expression, сравнение с switch в Java.
8. **Циклы — «повторение»** — `for` (по диапазону/коллекции), `while`, когда что использовать.
9. **Коллекции — «контейнеры для множества значений»** — `List`, `Set`, `Map`, иммутабельные vs мутабельные.
10. **Null — «пустота»** — null safety, `?.`, `?:`, `!!`, `let`.
11. **Наследование — «расширение класса»** — `open class`, переопределение методов, полиморфизм.
12. **Интерфейс — «контракт»** — аналогия с договором на работу, реализация интерфейсов, отличие от абстрактного класса.
13. **Корутины — «паузы в выполнении»** — аналогия с заказом пиццы, `suspend`, `delay` vs `Thread.sleep`.
14. **Общая картина** — сводная таблица всех концепций с аналогиями, рекомендации что читать дальше.

### Концептуальный подход

В отличие от остальных глав документации, здесь используется подход **«от понимания к синтаксису»**:

- **Сначала концепция** — что это такое, зачем нужно, аналогия из реальной жизни.
- **Потом синтаксис** — как это выглядит в Kotlin.
- **Потом расширение** — какие есть варианты, дополнения, когда использовать.

Пример (из раздела про функции):
> Представьте: у вас есть программа из 1000 строк. Если всё написать подряд — это будет нечитаемая каша. **Функция** — способ разбить программу на **именованные блоки**, каждый из которых делает одну вещь.
>
> Аналогия: **кнопка на пульте**. Вы не думаете о том, какие сигналы идут от пульта к телевизору. Вы нажимаете кнопку «громче» — громкость увеличивается. Кнопка — это **функция**: у неё есть имя («громче»), она что-то делает, и вам не нужно знать, как именно.

### INFO-блоки

Глава содержит 13 INFO-блоков, объясняющих встроенные конструкции Kotlin:
- Компилятор
- `val` vs `var`
- Type inference (когда писать тип явно)
- Статическая типизация
- Параметры функции как формальные аргументы в математике
- `Unit` и `return`
- Типы функций `(Int, Int) -> Int` и лямбды
- `val`/`var` в полях класса
- `==` vs `===`
- `data class`
- `when` vs switch
- `for` vs `while`
...
- `?` в типах (null safety)
- `open class` (почему классы по умолчанию финальные)
- Интерфейс vs абстрактный класс
- `suspend` и `delay` vs `Thread.sleep`

### Сводная таблица концепций

В конце главы — таблица «Концепция → Что это → Аналогия», охватывающая все 14 концепций:
...
1. **Новый раздел «🟢 Для начинающих (с нуля)»** — переименован из «Для начинающих», первым пунктом идёт новая глава.
2. **Quick Start обновлён** — первый шаг теперь: «Никогда не программировали? Начните с Программирования с нуля».
3. **Уровни сложности** — в строке «🟢 Начинающий» добавлена ссылка на новую главу.

## Обновления 00-introduction.md

1. В разделе «Как пользоваться этой документацией» добавлен INFO-блок: «Никогда не программировали? Начните с Программирования с нуля».
2. Навигационная строка обновлена: «← Программирование с нуля» вместо «← Словарь программиста».

## Итог 6-го прохода

| Что | Объём |
|-----|-------|
| Новая глава `00-programming-foundations.md` | 827 строк, 14 разделов, 13 INFO-блоков |
| README.md | 3 правки (новый раздел, Quick Start, уровни сложности) |
| 00-introduction.md | 2 правки (INFO-блок, навигация) |
| **Итого** | ~830 строк нового концептуального контента |

## Принцип

Глава следует принципу **«от понимания к синтаксису»**:
1. Сначала концепция (что это, зачем, аналогия).
2. Потом синтаксис (как в Kotlin).
3. Потом расширение (варианты, когда использовать).

Это противополагается традиционному подходу справочника («вот так пишут класс, вот так функцию») и помогает новичкам **понимать**, а не **запоминать**.

---

# Часть 7: Tink, P2P-сети, [!TIP] вместо [!INFO] (30 июня 2026)

Пользователь предоставил файлы проекта ReMatch Chat — гибридного мессенджера с Хабом, Direct-talk (P2P через QUIC), Посредниками (STUN/TURN/WebSocket), канальными серверами и синхронизацией между устройствами. На основе этого контекста добавлены:

1. Заменены все `> [!INFO]` на `> [!TIP]` (по просьбе пользователя).
2. Новая секция 12.3 «Google Tink — высокоуровневая криптография» в главе 12.
3. Новая глава 38 «P2P-сети и мессенджеры» — самое большое добавление за весь цикл.

## Изменение 1: [!INFO] → [!TIP]

Все 28 INFO-блоков в 10 главах заменены на TIP. Затронутые файлы:
- 00-programming-foundations.md (16 блоков)
- 01-kotlin-basics.md (3)
- 32-state-management-deep-dive.md (2)
- 20-deep-dive-kotlin.md (1)
- 10-performance.md (1)
- 09-testing.md (1)
- 08-navigation.md (1)
- 06-networking-api.md (1)
- 05-data-storage.md (1)
- 00-beginners-guide.md (1)

## Изменение 2: Глава 12 — добавлена секция 12.3 «Google Tink»

Старая секция 12.2 (Шифрование данных) расширена, добавлена новая секция 12.3 про Google Tink:

- **Зачем Tink** — TIP-блок с 4 причинами (мультиязыковость, ротация ключей, авто-IV, безопасность по умолчанию).
- **Подключение** — версия 1.16.0, зависимость.
- **Статус (2026)** — Tink работает на JVM/Android, для iOS/Wasm рекомендован kotlin-multiplatform-libsodium.
- **Симметричное шифрование AES-GCM** — полный пример: AeadConfig.register(), generateNew, getPrimitive, encrypt/decrypt с associatedData.
- **TIP-блок про AAD** — что такое associatedData, для чего нужно (привязка шифртекста к контексту).
- **Хранение ключей: Keyset** — сериализация в JSON, предупреждение о безопасности.
- **Ротация ключей** — пример через KeysetManager, объяснение зачем нужна ротация.
- **Гибридное шифрование ECIES** — для мессенджеров: DHKEM_X25519_HKDF_SHA256_HKDF_SHA256_AES_256_GCM.
- **Сравнение крипто-решений для KMP** — таблица: Tink / libsodium / Bouncy Castle / javax.crypto / CryptoKit / Web Crypto API.

Старая секция 12.3 (Защита от уязвимостей) сдвинута в 12.4.

## Изменение 3: Новая глава 38 «P2P-сети и мессенджеры»

Самое большое добавление — 550+ строк, 11 разделов, охватывающих все аспекты P2P-мессенджера:

### 38.1. Архитектуры сетевого взаимодействия
- **Клиент-сервер** — централизованная, плюсы/минусы.
- **P2P** — децентрализованная, плюсы/минусы.
- **Гибридная** — сигналинг + P2P + fallback через Посредника (как в ReMatch Chat).

### 38.2. QUIC — транспортный протокол нового поколения
- TIP-блок «QUIC vs TCP — что лучше для мессенджера?».
- Таблица поддержки QUIC в KMP (Android Cronet ✅, iOS Darwin ✅, JVM ⚠️, Wasm ⚠️).
- Полный пример QUIC-клиента через Cronet на Android.
- Прямое QUIC-соединение через UDP (DatagramChannel) с expect/actual.

### 38.3. NAT Traversal: STUN, TURN, ICE
- **STUN** — обнаружение публичного адреса. TIP про ограничения (symmetric NAT).
- **TURN** — ретрансляция трафика. TIP: «TURN не нарушает E2E-шифрование».
- **ICE** — оркестрация кандидатов (host > server reflexive > relay).
- Реализация в Ktor WebRTC Client.
- Самостоятельный TURN-сервер (coturn) с конфигурацией.
- TIP: «Свой TURN — необходимость для production».

### 38.4. Сигналинг — установление P2P-соединения
- Сигналинг через WebSocket (Ktor client).
- Полный пример SignalingClient с SharedFlow и sealed class SignalingMessage.
- Сервер сигналинга (Ktor server) с WebSocket routing.
- TIP: «Сигналинг-сервер хранит минимум данных».

### 38.5. End-to-End шифрование (E2E)
- Double Ratchet Algorithm (Signal Protocol) — forward secrecy, future secrecy, out-of-order.
- Реализация через kotlin-multiplatform-libsodium (KMP).
- Полный пример: generateKeyPair, encrypt, decrypt через Box.easy.
- TIP: что такое crypto_box (X25519+XSalsa20+Poly1305).
- **Safety numbers** для верификации ключей — пример кода.

### 38.6. Синхронизация между устройствами
- **Вариант 1: через Посредника** — пример SyncService с LZ4 сжатием и AES-GCM шифрованием.
- **Вариант 2: локальная через mDNS** — expect/actual для Android NSD.
- **Вариант 3: USB-синхронизация** — через Android USB Host API.

### 38.7. Сжатие данных (LZ4)
- KMP-библиотека для LZ4 (lz4-java на JVM/Android).
- Полный expect/actual пример Compression object.
- TIP: «Сжатие + шифрование — порядок важен!» — сжать → зашифровать → отправить.

### 38.8. Push-уведомления в P2P-приложении
- Архитектура Notification Service (как в ReMatch Chat).
- Полный пример: NotificationService + FirebaseMessagingService.
- TIP: «Notification Service видит только зашифрованные данные».

### 38.9. Канальные сервера (федерация)
- Архитектура федерации (как в Matrix/XMPP).
- Преимущества/недостатки федерации.
- Простая федерация через HTTP с подписью запроса (HMAC-SHA256).

### 38.10. Чеклист production P2P-мессенджера
- 12 пунктов: сигналинг, STUN, TURN, E2E, safety numbers, push, синхронизация, сжатие, ротация ключей, защита от replay, обработка офлайна, мониторинг.

### 38.11. Дополнительные ресурсы
- RFC 9000 (QUIC), RFC 8445 (ICE), RFC 8489 (STUN), RFC 8656 (TURN).
- Signal Protocol Documentation.
- Cronet, coturn, libsodium, Tink, Matrix Protocol — ссылки.

## Сводка 7-го прохода

| Что | Объём |
|-----|-------|
| Замена [!INFO] → [!TIP] | 28 блоков в 10 главах |
| Глава 12 (Безопасность) | +140 строк (секция 12.3 про Tink) |
| Новая глава 38 (P2P-сети) | 550 строк, 11 разделов |
| README | +1 строка (новая глава в оглавлении) |
| **Итого** | ~700 строк нового технического контента + 28 правок |

## Темы, релевантные для ReMatch Chat

Глава 38 закрывает большинство технических вопросов из RMC.md:
- ✅ Direct-talk через QUIC (раздел 38.2).
- ✅ Fallback на TLS через Хаб (раздел 38.4 сигналинг).
- ✅ Посредники как TURN (раздел 38.3).
- ✅ WebSocket и STUN/TURN (раздел 38.3).
- ✅ Синхронизация между устройствами через Посредника / mDNS / USB (раздел 38.6).
- ✅ LZ4 сжатие + шифрование (раздел 38.7).
- ✅ Notification Service на базе Посредника (раздел 38.8).
- ✅ Канальные сервера и федерация (раздел 38.9).
- ✅ E2E-шифрование через libsodium/Tink (раздел 38.5 + глава 12.3).
